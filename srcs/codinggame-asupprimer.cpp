...
int main()
{
    ...
    while (1) {
        ...
        if (flag_many_ways == 1)
        {
            ...
            {
                if(compar_way[0].size()  >= compar_way[1].size())
                     actions += compar_way[1];
                else                
                     actions += compar_way[0];

            }
                cerr<< "best way nid de meme valeur: "<<actions << endl;
        }
        // string ant_cell_str = to_string(ant_cell._index);
        // string ant_cell_str2 = to_string(ant_cell2._index);
        ...
        for (int i = 0; i < number_of_cells; i++) 
        {
            if (cells[i]._cellType == 2)
            {
                if(cells[i]._resources == big_cell._resources)
                {
                        flag_many_ways_cristaux = 1;
                        string big_cell_str = to_string(big_cell._index);//recup la 1ere valeur 
                         best_way =  "LINE " + base + " " + big_cell_str + " 4;";
                    // cerr<<"best way si = " <<best_way<< endl;
                        compar_way.push_back(best_way);
                        big_cell = cells[i];
                        big_cell_str = to_string(big_cell._index);
                    // cerr<<"best way si = " <<best_way<< endl;
                        best_way =  "LINE " + base + " " + big_cell_str + " 4;";
                        compar_way.push_back(best_way);
                    // cerr<<"best way si = " <<best_way<< endl;
                }
                else if(cells[i]._resources > big_cell._resources)
                { 
                    big_cell = cells[i];
                    string big_cell_str = to_string(big_cell._index);
                    actions =  "LINE " + base + " " + big_cell_str + " 4;";
                    cerr<< "cristaux1 "<<actions << endl;
                }
                else if(cells[i]._resources >= big_cell_2._resources)
                {
                    big_cell_2 = cells[i];
                    string big_cell_str2 = to_string(big_cell_2._index);
                    if(big_cell_2._resources != 0)//si jai une 2eme source
                        actions +=  "LINE " + base + " " + big_cell_str2 + " 1;";
                    cerr<< "cristaux2 "<<actions << endl;
                }
